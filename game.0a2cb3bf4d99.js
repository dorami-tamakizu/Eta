...
function updateBoss(e,dt){
  ...
  const oldStep=e.stepClock||0,newStep=oldStep+dt/.95;
...
function drawBossSprite(e,h){
  ...
  drawBossAura(e,h);
  const sheet=im.bossOverhead;
  if(sheet.complete&&sheet.naturalWidth){
    const striking=a&&a.age>=a.impact&&a.age<a.impact+.32;
    const frame=striking?(a.age<a.impact+.20?6:7):(e.stepFrame||0);
    const cw=sheet.naturalWidth/4,ch=sheet.naturalHeight/2,w=h*cw/ch;
    c.drawImage(sheet,(frame%4)*cw,Math.floor(frame/4)*ch,cw,ch,-w/2,-h,w,h);
