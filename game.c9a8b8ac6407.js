...
function drawCinematic(){
  ...
  if(kind==='entry'){
    ...
    if(t<1.6){
      const img=im[BOSS_PRESENTATION.image],q=C(t/1.6,0,1);
      c.fillStyle='rgba(10,21,25,.45)';c.fillRect(0,0,W,H);
      if(img.complete&&img.naturalWidth){const rect=crop(img),h=H*.40*(1-.65*E(q)),w=H*.40*rect[2]/rect[3]*(1+.15*q);c.globalAlpha=1-q;c.drawImage(img,...rect,W/2-w/2,H*.64-h,w,h);c.globalAlpha=1;}
    }else cinematicRibbon('QUEST CLEAR','クエストクリア',H*.40,Math.min(1,(t-1.6)/.4));
...
function draw(){
  ...
  for(const e of S.en.filter(e=>(!S.cinematic||!e.boss)&&(!e.dead||e.fade>0)&&e.z-S.z>-2&&e.z-S.z<45).sort((a,b)=>b.z-a.z)){
    ...
    if(e.fl)c.globalAlpha*=.6;if(e.boss)drawBossSprite(e,h);else c.drawImage(img,...rect,-w/2,-h,w,h);c.restore();
