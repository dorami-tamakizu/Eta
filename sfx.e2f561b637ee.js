...
(()=>{
  ...
  const durations={waterStart:.18,waterSwing:.24,continentStart:.23,continentSwing:.34,hit:.12,down:.43,shot:.28,hurt:.20,battleStart:.75,guardRaise:.18,guardBlock:.24};
    ...
    for(let i=0;i<out.length;i++){
      ...
      switch(kind){
        ...
        case 'battleStart': {
          ...
          value=notes.reduce((v,n,j)=>{const a=t-j*.095;return v+(a>0?.17*Math.sin(2*Math.PI*n*a)*Math.exp(-4*a):0)},0)+.14*low*Math.exp(-14*t);break;
        }
