...
function adv(n){
  if(S.phase==='boss'){
    if(S.dash){const gap=Math.max(0,front()-S.z-S.pd-FRONT_GAP),travel=Math.min(n,gap),local=Math.min(travel,Math.max(0,MAX_DEPTH-S.pd));S.pd+=local;S.pt=S.pd;S.z+=travel-local;return travel;}
...
function accelerate(){
  ...
  if(S.dashUntil>=0&&S.t<=S.dashUntil+1e-9){
    S.retreat=0;S.dash={n:Infinity};sound('superdash');S.dashUntil=-1;S.boost=0;
  }else S.boost=1;
}
...
function updateBoss(e,dt){
  ...
  const p=e.patrol,target=route[p.leg],duration=Math.max(4,Math.abs(target[0]-p.fromLane)/.42,Math.abs(target[1]-p.fromDepth)/.85);
  // Each deliberate footstep has a planted pause and a short weight-transfer phase.
  const oldStep=e.stepClock||0,newStep=oldStep+(e.action?0:dt/.95);
  const progress=t=>Math.floor(t)+C(((t%1)-.22)/.40,0,1);
  p.t=Math.min(duration,p.t+(progress(newStep)-progress(oldStep))*.95);e.stepClock=newStep;
  ...
  if(e.action){
    ...
    if(a.age>=a.duration){e.recover={kind:a.kind,age:0};e.action=null;e.tell=0;e.cd=.65;}
    return;
  }
  if(e.wideCd<=0){
...
function bossVisual(e){
  const a=e.action;
  if(!a)return {sheet:im.bossOverhead,frame:((e.stepClock||0)*4)%8};
  const dragon=a.kind==='dragon',sheet=dragon?im.bossSlam:im.bossSweep;
  // Contact pose and damage share the exact 0.5-second impact clock.
  const contact=dragon?5:4;
  const frame=a.age<a.impact?C(a.age/a.impact*contact,0,contact):
    contact+C((a.age-a.impact)/.30,0,1)*(6-contact);
  return {sheet,frame};
}
function drawBossFrame(sheet,frame,h,alpha=1){
  ...
  c.drawImage(sheet,frame%4*cw,Math.floor(frame/4)*ch,cw,ch,-w/2,-h,w,h);c.restore();
}
function drawBossVeil(h,strength,t){
  if(strength<=0)return;
  c.save();c.globalAlpha*=C(strength,0,1);c.shadowColor='#ff6414';c.shadowBlur=10;
  for(let i=0;i<11;i++){
    const x=(i-5)*h*.055,w=h*.065,tip=-h*(.65+.22*Math.sin(i*1.7+t*19)**2);
    const g=c.createLinearGradient(0,0,0,tip);g.addColorStop(0,'rgba(255,70,5,.15)');g.addColorStop(.45,'rgba(255,105,15,.8)');g.addColorStop(.78,'rgba(255,210,90,.55)');g.addColorStop(1,'rgba(255,180,55,0)');c.fillStyle=g;
    c.beginPath();c.moveTo(x-w,0);c.bezierCurveTo(x-w*2,tip*.45,x+w,tip*.7,x,tip);c.bezierCurveTo(x+w*2,tip*.85,x-w*2,tip*.35,x+w,0);c.fill();
  }c.restore();
}
function drawBossSprite(e,h){
  drawBossAura(e,h);
  const v=bossVisual(e),a=e.action,r=e.recover;
  const frame=Math.floor(v.frame),mix=C((v.frame-frame-.72)/.28,0,1);
  const recovery=r?C(r.age/.22,0,1):1;
  if(r){const sheet=r.kind==='dragon'?im.bossSlam:im.bossSweep;drawBossFrame(sheet,6,h,1-recovery);}
  drawBossFrame(v.sheet,frame,h,(1-mix)*recovery);
  if(mix)drawBossFrame(v.sheet,(frame+1)%8,h,mix*recovery);
  if(a){
...
function drawBossWideSlash(){
  ...
  const frame=Math.min(3,Math.floor(q*4)),sw=img.naturalWidth,sh=img.naturalHeight/4,w=(right-left)*1.08,h=w/6;
  c.save();c.globalAlpha=q>.78?(1-q)/.22:1;
  c.drawImage(img,0,frame*sh,sw,sh,W/2-w/2,y-h/2,w,h);c.restore();
}
...
function step(dt){
  ...
  if(!S.guard&&!S.ultimate){
    if(S.dash){const requested=42*dt,v=adv(requested);if(v>.0001&&!S.dash.rewarded){S.dash.rewarded=true;for(let i=0;i<2;i++){S.ch[i]=Math.min(4,S.ch[i]+1);if(S.ch[i]===4)S.fr[i]=0}}if(!Number.isFinite(front())||front()-S.z-S.pd<=FRONT_GAP+.025)S.dash=null}
...
function draw(){
  ...
  for(const e of S.en.filter(e=>(!S.cinematic||!e.boss||(S.cinematic.kind==='victory'&&S.cinematic.t<1.6))&&(!e.dead||e.fade>0)&&e.z-S.z>-2&&e.z-S.z<45).sort((a,b)=>b.z-a.z)){
    const z=e.z-S.z,p=perspective(z),img=im[e.t],x=lx(e.boss?e.displayLane:e.l,z),y=yy(z),h=e.boss?Math.max(D[e.t].h*p*Math.min(1,H/844)*.77*1.85,playerHeight()*1.55):D[e.t].h*p*Math.min(1,H/844)*.77;
