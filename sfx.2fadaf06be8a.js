// CC0 sword clash cue (StarNinjas) and procedural gameplay effects.
(()=>{
  let context=null,bus=null,enabled=false,epoch=0;
  const battleAudio=new Audio('assets/sword-clash-cc0-v7.m4a');battleAudio.preload='auto';battleAudio.volume=.12;
  const ultimateAudio=new Audio('assets/sword-clash-cc0-v7.m4a');ultimateAudio.preload='auto';let resumeUltimate=false;
  const buffers=new Map(),voices=new Set(),recent=new Map();
  const durations={ultimateCutin:1.324,waterStart:.18,waterSwing:.24,continentStart:.23,continentSwing:.34,hit:.12,down:.43,shot:.28,hurt:.20,battleStart:1.324,superdash:.48,guardRaise:.18,guardBlock:.24};
  ...
  function play(kind,type=''){
    ...
    try{
      if(kind==='ultimateCutin'){ultimateAudio.currentTime=Math.max(0,Math.min(1.324,Number(type)||0));ultimateAudio.play().catch(()=>{});return}
