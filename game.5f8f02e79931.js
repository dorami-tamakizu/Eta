...
const ULTIMATE={capacity:30,damage:10,duration:1.87,cutin:.72,windup:.90,travel:.32};
function updateUltimateUI(){
  const value=C(S.ultimateCharge,0,30),button=$('#ultimate');
  $('#ultimateFill').style.strokeDashoffset=String(100*(1-value/30));
  button.setAttribute('aria-label','奥義・渾身のストラッシュ：ゲージ'+Math.floor(value)+'/30');
  button.setAttribute('aria-disabled',String(value<30));
}
function fireUltimate(){
  ...
  S.ptr=null;S.dash=null;S.boost=0;S.ultimate={t:0,origin:S.z+S.pd,far:S.z+45,seen:new Set()};hideNotice();sound('waterStart');updateUltimateUI();return true;
}
function updateUltimate(dt){
  const a=S.ultimate;if(!a)return;a.t+=dt;
  const progress=C((a.t-ULTIMATE.windup)/ULTIMATE.travel,0,1);
  if(progress>0){
    if(!a.fired){a.fired=true;notice('渾身のストラッシュ',.85);sound('waterSwing');}
...
function drawUltimateCutin(t){
  const enter=E(C(t/.14,0,1)),exit=E(C((t-.52)/.20,0,1));
  const visibility=enter*(1-exit),slide=(1-enter)*W*1.12-exit*W*1.12;
  const bandH=Math.min(H*.30,W*.66),cy=H*.43,tilt=W*.13;
  c.save();c.fillStyle='rgba(4,7,20,'+(.62*visibility)+')';c.fillRect(0,0,W,H);
  c.translate(slide,0);
  const band=()=>{c.beginPath();c.moveTo(-W*.2,cy-bandH/2+tilt);c.lineTo(W*1.2,cy-bandH/2-tilt);c.lineTo(W*1.2,cy+bandH/2-tilt);c.lineTo(-W*.2,cy+bandH/2+tilt);c.closePath();};
  c.save();band();c.clip();
  const bg=c.createLinearGradient(0,cy,W,cy);bg.addColorStop(0,'#14113c');bg.addColorStop(.52,'#623291');bg.addColorStop(1,'#162653');c.fillStyle=bg;c.fillRect(-W,cy-bandH,W*3,bandH*2);
  c.strokeStyle='#b8edff';c.lineWidth=1.5;c.globalAlpha=.45;
  for(let i=0;i<18;i++){const y=cy-bandH+i*bandH/9,x=((i*79+t*1100)%(W*1.8))-W*.4;c.beginPath();c.moveTo(x,y);c.lineTo(x+W*.38,y-W*.06);c.stroke();}
  c.globalAlpha=1;
  const img=im.ultimateFace;
  if(img.complete&&img.naturalWidth){
    const width=W*1.22*(1+.035*C(t/.72,0,1)),height=width*img.naturalHeight/img.naturalWidth;
    // Keep the eyes and brave expression centered inside the diagonal band.
    c.drawImage(img,W*.52-width*.52,cy-height*.46,width,height);
  }
  c.restore();band();c.strokeStyle='#d2a7ff';c.lineWidth=3;c.stroke();
  c.strokeStyle='#f1edff';c.lineWidth=1;c.stroke();c.restore();
}
function drawUltimate(){
  const a=S.ultimate;if(!a)return;
  if(a.t<ULTIMATE.cutin){drawUltimateCutin(a.t);return;}
  const launch=C((a.t-ULTIMATE.cutin)/(ULTIMATE.windup-ULTIMATE.cutin),0,1),p=C((a.t-ULTIMATE.windup)/ULTIMATE.travel,0,1);
  const fade=1-C((a.t-ULTIMATE.cutin-.60)/.55,0,1);
  c.save();c.fillStyle='rgba(7,18,28,'+(.18*fade)+')';c.fillRect(0,0,W,H);
  c.globalCompositeOperation='lighter';
  for(let lane=0;lane<3;lane++){
    const startX=W/2+(lane-1)*W*.39,startY=H*.88;
    const endX=W/2+(lane-1)*W*.038,endY=H*(HORIZON+.008);
    if(p<=0){c.strokeStyle='#bcffde';c.lineWidth=2;c.globalAlpha=launch*.7;c.beginPath();c.ellipse(startX,H*.70,12+launch*16,4+launch*7,0,0,Math.PI*2);c.stroke();continue;}
    const tipX=startX+(endX-startX)*p,tipY=startY+(endY-startY)*p;
    for(const [width,color,alpha] of [[W*.065,'#4fe4a2',.27],[W*.024,'#91fff0',.8],[W*.006,'#ffffff',1]]){
      c.globalAlpha=fade*alpha;c.fillStyle=color;c.beginPath();c.moveTo(startX-width,startY);c.lineTo(startX+width,startY);c.lineTo(tipX+width*.07,tipY);c.lineTo(tipX-width*.07,tipY);c.closePath();c.fill();
    }
    for(let j=0;j<9;j++){
      const t=((a.t*2+j/9)%1)*p,x=startX+(endX-startX)*t,y=startY+(endY-startY)*t;
      const span=(1-t)*W*.03+2;
      c.globalAlpha=fade*.75;c.strokeStyle=j%2?'#e5ffbd':'#b3fff8';c.lineWidth=1.3;c.beginPath();c.moveTo(x-span,y+span*.6);c.lineTo(x,y);c.lineTo(x+span,y+span*.6);c.stroke();
    }
  }
  c.restore();
}
function fire(k){
  if(!S.run||S.pause||S.cinematic||S.ultimate||S.skill||S.ch[k-1]<1)return;
...
function revealBoss(){
  const boss=mk(BOSS_PRESENTATION.image,S.z+S.pd+18);
...
function updateBoss(e,dt){
  e.displayLane+=(e.l-e.displayLane)*(1-Math.exp(-9*dt));
  if(e.action){
    const a=e.action;
    if(a.kind==='dragon'&&!a.done)e.z+=C(S.z+S.pd+8-e.z,-5*dt,14*dt);
    a.age+=dt;e.tell=Math.max(0,a.impact-a.age);
    if(!a.done&&a.age>=a.impact){
      a.done=true;
      if(a.kind==='dragon'){
        S.shots.push({kind:'fireDragon',l:a.l,z:e.z,d:4,t:'boss',age:0});sound('shot','dragon');
      }else{
        sound('continentSwing');burst(a.l,e.z,'#ffad38',12);
        if(S.l===a.l&&distance(e)>=0&&distance(e)<=3)hurt(3);
      }
    }
    if(a.age>=a.duration){e.action=null;e.tell=0;e.cd=1.1;}
  }else{
    e.cd-=dt;
    if(e.cd<=0){
      const kind=distance(e)>4||e.attackCount%3===2?'dragon':'sword';
      e.attackCount++;e.l=S.l;
      e.action={kind,l:S.l,age:0,impact:kind==='dragon'?1.5:1.1,duration:kind==='dragon'?2.1:2.0,done:false};
      e.tell=e.action.impact;
    }
  }
  // Approach for sword range; create room after two sword attacks for the fire dragon.
  if(!e.action){const gap=e.attackCount%3===2?8:1.5,target=S.z+S.pd+gap;e.z+=C(target-e.z,-5*dt,14*dt);}
}
function drawBossSprite(e,h){
  ...
  if(a&&atlas.complete&&atlas.naturalWidth){
    let frame;
    if(a.kind==='dragon')frame=a.age<a.impact?Math.min(2,Math.floor(a.age/.3)):6;
  ...
  if(a&&a.kind==='sword'&&a.age>=a.impact&&a.age<a.impact+.25){
...
function drawBossTelegraph(){
  const e=S.en.find(e=>e.boss&&!e.dead),a=e&&e.action;if(!a||a.done||S.cinematic)return;
  const near=S.pd+.1,far=e.z-S.z,half=Math.min(W,520)*.115;
  c.save();c.fillStyle='rgba(255,65,18,'+(.10+.10*(a.age/a.impact))+')';c.strokeStyle='#ffa244';c.lineWidth=1.5;
  c.beginPath();c.moveTo(lx(a.l,near)-half*perspective(near),yy(near));c.lineTo(lx(a.l,near)+half*perspective(near),yy(near));c.lineTo(lx(a.l,far)+half*perspective(far),yy(far));c.lineTo(lx(a.l,far)-half*perspective(far),yy(far));c.closePath();c.fill();c.stroke();
  c.font='bold 14px sans-serif';c.textAlign='center';c.fillStyle='#ffe2a4';c.fillText(a.kind==='dragon'?'炎竜':'剣撃',lx(a.l,near),yy(near)+20);c.restore();
}
...
function step(dt){
  ...
  for(const p of S.shots){
    p.z-=(p.kind==='fireDragon'?10:18)*dt;p.age=(p.age||0)+dt;
...
function draw(){
  ...
  if(armoredReady())drawArmoredHero(h);else if(img.complete&&img.naturalWidth){const rect=crop(img),w=h*rect[2]/rect[3];c.drawImage(img,...rect,-w/2,-h,w,h)}c.restore();drawWaterEffect(true);drawEffects();c.restore();drawUltimate();drawBattleBanner();
  if(S.hit){c.fillStyle='rgba(255,80,60,.12)';c.fillRect(0,0,W,H)}
  drawCinematic();
}
const SCORE_KEY='eta.score.v3.boss';
function score(ok){
...
function end(ok){
  ...
  $('#resultText').innerHTML=
    ...
    resultRow('与ダメージスコア','与ダメージ',(S.dealtDamage+S.overkill).toLocaleString('ja-JP'),points.dealt+points.overkill);
