...
(()=>{
  ...
  const durations={waterStart:.18,waterSwing:.24,continentStart:.23,continentSwing:.34,hit:.12,down:.43,shot:.28,hurt:.20,battleStart:.90,superdash:.48,guardRaise:.18,guardBlock:.24};
  function synth(kind,type,rate){
    ...
    for(let i=0;i<out.length;i++){
      ...
      switch(kind){
        ...
        case 'battleStart': {
          // Fast impact plus inharmonic steel resonances: a single bright clang.
          const strike=.23*(noise-low)*Math.exp(-170*t);
          value=strike+.24*Math.sin(2*Math.PI*1480*t)*Math.exp(-2.8*t)
            +.15*Math.sin(2*Math.PI*2395*t)*Math.exp(-4.2*t)
            +.09*Math.sin(2*Math.PI*3910*t)*Math.exp(-7*t)
            +.06*Math.sin(2*Math.PI*6270*t)*Math.exp(-15*t);break;
        }
