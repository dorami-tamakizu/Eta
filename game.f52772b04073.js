...
const ENEMY_ROW_GAP=3,SUPERDASH_DISTANCE=ENEMY_ROW_GAP*2,SUPERDASH_SPEED=42;
...
function bossVisual(e){
  ...
  const times=a.kind==='dragon'?[0,a.impact-.20,a.impact,a.impact+.16]:[0,a.impact-.30,a.impact-.14,a.impact];
...
function drawFireDragon(shot){
  ...
  const bossH=Math.max(300*perspective((shot.originZ??shot.z)-S.z)*Math.min(1,H/844)*.77*1.85,playerHeight()*1.55);
...
function step(dt){
  ...
  if(!S.guard&&!S.ultimate){
    ...
    if(S.dash){
      ...
      if(dash.n<=1e-9||!Number.isFinite(front())||front()-S.z-S.pd<=FRONT_GAP+.025){
