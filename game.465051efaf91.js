...
function updateWaves(dt){
  for(const w of S.waves){
    ...
    for(const e of S.en){
      ...
      if(w.k===1?Math.abs(e.l-w.l)<=1:e.l===w.l){w.seen.add(e);hit(e,w.k===1?2:3,1);if(w.k===1)S.fx.push({kind:'impact',l:e.l,z:e.z,t:0,life:.32,color:'#b2fff3'});if(w.k===2)S.fx.push({kind:'impact',l:e.l,z:e.z,t:0,life:.36,color:'#c4f8ff'})}
    }
  }
  S.waves=S.waves.filter(w=>w.travel<(w.k===1?9:13));
}
// A broad translucent water blade, bright foam edge and discrete spray.
// Geometry is generated locally; combat RNG and hit boxes are unaffected.
function waterBlade(radius,phase,opacity=1){
  c.save();c.globalAlpha*=opacity;c.rotate(-.10);
  const depth=radius*.40,thick=radius*.19;
  const fill=c.createLinearGradient(0,-depth,0,thick);
  fill.addColorStop(0,'rgba(236,255,255,.94)');
  fill.addColorStop(.35,'rgba(103,255,250,.85)');
  fill.addColorStop(1,'rgba(12,158,232,.10)');
  c.fillStyle=fill;c.shadowColor='#5affef';c.shadowBlur=12;
  c.beginPath();c.moveTo(-radius,0);
  c.bezierCurveTo(-radius*.60,-depth*1.5,radius*.55,-depth*1.5,radius,0);
  c.bezierCurveTo(radius*.60,-depth+thick,-radius*.45,-depth+thick,-radius,0);
  c.fill();
  c.shadowBlur=4;c.strokeStyle='#e5ffff';c.lineWidth=Math.max(1.5,radius*.025);
  c.beginPath();c.moveTo(-radius,0);
  c.bezierCurveTo(-radius*.60,-depth*1.5,radius*.55,-depth*1.5,radius,0);c.stroke();
  for(let i=0;i<13;i++){
    const a=Math.PI*(.08+i*.07),f=(phase*1.8+i*.173)%1;
    const x=Math.cos(a)*radius*(.76+f*.30),y=-Math.sin(a)*depth-f*radius*.20;
    c.strokeStyle=i%3?'#b8fff3':'#ffffff';c.lineWidth=1.2+(i%3)*.5;
    c.beginPath();c.moveTo(x,y);c.lineTo(x+Math.cos(a)*radius*.04,y-radius*(.035+f*.04));c.stroke();
  }
  c.restore();
}
function drawWaves(){for(const w of S.waves){
  ...
  if(w.k===1){
    c.save();c.translate(x,y-35*p);
    const q=w.travel/9,alpha=C((1-q)*2,0,1),radius=Math.min(W,520)*(.34+.10*q)*p;
    waterBlade(radius,q,alpha);
    c.translate(0,12*p);c.scale(.94,.72);waterBlade(radius,q+.2,alpha*.23);
    c.restore();continue;
  }
  if(w.k===2){
...
function drawPoseHero(h){
  ...
  if(skill.k===1){
    const q=(p-.43)/.48;if(q<0||q>1)return;
    c.save();c.translate(0,-h*.45);
    c.rotate(-.38+q*.68);
    waterBlade(h*(.65+.45*Math.sin(q*Math.PI)),q,Math.sin(q*Math.PI)*.85);
    c.restore();return;
  }
