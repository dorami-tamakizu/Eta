...
function armoredFrame(sheet,index,h,running=false,alpha=1){
  ...
  c.save();c.globalAlpha*=alpha;
  c.drawImage(sheet,(index%4)*cw,Math.floor(index/4)*ch,cw,ch,-a[0]*scale,-a[1]*scale,cw*scale,ch*scale);
  c.restore();
}
...
function draw(){
  ...
  if(armoredReady())drawArmoredHero(h);else if(img.complete&&img.naturalWidth){const rect=crop(img),w=h*rect[2]/rect[3];c.drawImage(img,...rect,-w/2,-h,w,h)}c.restore();drawWaterEffect(true);drawEffects();c.restore();
