...
function updateBoss(e,dt){
  ...
  // Smooth forward/back patrol between center and four depth units farther away.
  if(e.patrolTime===undefined)e.patrolTime=Math.acos(C(1-(e.z-centerZ)/2,-1,1))/.28;
  e.patrolTime+=dt;
  e.z=centerZ+2*(1-Math.cos(e.patrolTime*.28));
  e.l=1;e.displayLane+=(1-e.displayLane)*(1-Math.exp(-9*dt));
  const movement=e.z-beforeMove;e.moveActive=Math.abs(movement)>1e-6;e.backstepActive=movement>1e-6;
  if(e.moveActive)e.backstepPhase=((e.backstepPhase||0)+movement/.65+1)%1;
...
function drawBossSprite(e,h){
  ...
  if(!a&&e.moveActive&&walk.complete&&walk.naturalWidth){
  ...
  }else if(!e.moveActive||!walk.complete||!walk.naturalWidth){
