...
function step(dt){
  if(!S.run||S.pause)return;
  const previousZ=S.z+S.pd;S.t+=dt;S.ll+=(S.l-S.ll)*(1-Math.exp(-18*dt));
  ...
  if(S.skill){
    ...
    if(!S.skill.done&&p>(S.skill.k===1?.48:.54)){
      ...
      if(S.skill.k===1)S.waterFx.push({age:0,l:S.l});
...
function drawWaterEffect(frontPass){
  for(const f of S.waterFx){
    const t=f.age,q=waterState(t),h=playerHeight(),rx=h*(.62+.18*q.spread),ry=h*.19;
    c.save();c.translate(lx(f.l,S.pd),playerY()-h*.48);c.rotate(-.055);
    // The far half is drawn before the body, the near half after it.
    if(!frontPass&&q.lift>0){
      const curtain=c.createLinearGradient(0,-h*.62,0,-ry*.25);
      curtain.addColorStop(0,'rgba(114,255,247,0)');
      curtain.addColorStop(.62,'rgba(109,255,244,.35)');
      curtain.addColorStop(1,'rgba(202,255,255,.72)');
      c.fillStyle=curtain;c.globalAlpha=q.lift;
      c.beginPath();c.moveTo(-rx*.78,-ry*.25);
      for(let i=0;i<=18;i++){
        const x=(-.78+i*1.56/18)*rx;
        const crest=(.22+.20*Math.sin(i*2.37)**2)*h*q.lift;
        c.quadraticCurveTo(x-rx*.045,-ry*.5-crest*.35,x,-ry*.5-crest);
      }
      c.lineTo(rx*.78,-ry*.25);c.quadraticCurveTo(0,-ry*1.7,-rx*.78,-ry*.25);c.fill();
      for(let i=0;i<9;i++){
        const x=(i/8-.5)*rx*1.42;
        c.strokeStyle=i%3?'#8cfdf466':'#ecffffaa';c.lineWidth=h*(i%3?.022:.038);
        c.beginPath();c.moveTo(x,-ry*.75);c.lineTo(x+h*.03,-ry*.75-h*(.16+(i%3)*.08)*q.lift);c.stroke();
      }
    }
    const steps=48,start=frontPass?Math.PI:Math.PI*2;
    const end=start-Math.PI*q.spread;
    const nearFade=frontPass?C((.46-t)/.18,0,1):1;
    const thickness=h*(frontPass?.17:(.06+.10*C((t-.20)/.12,0,1)))*q.alpha;
    if(q.alpha>0&&q.spread>0){
      c.globalAlpha=q.alpha*nearFade*(frontPass?1:.72);
      const gradient=c.createLinearGradient(0,-ry,0,ry+h*.1);
      gradient.addColorStop(0,'#b9fff666');gradient.addColorStop(.6,'#5ffbf1dd');gradient.addColorStop(1,'#efffff');
      c.fillStyle=gradient;c.shadowColor='#62fdf2';c.shadowBlur=8;
      c.beginPath();
      for(let i=0;i<=steps;i++){const a=start+(end-start)*i/steps;
        const x=Math.cos(a)*rx,y=Math.sin(a)*ry;i?c.lineTo(x,y):c.moveTo(x,y)}
      for(let i=steps;i>=0;i--){const a=start+(end-start)*i/steps;
        const taper=Math.sin(Math.PI*i/steps)*.85+.15;
        c.lineTo(Math.cos(a)*rx*.99,Math.sin(a)*ry-thickness*taper)}
      c.closePath();c.fill();
      c.shadowBlur=0;c.strokeStyle='#e6ffff';c.lineWidth=Math.max(1,h*.012);c.beginPath();
      for(let i=0;i<=steps;i++){const a=start+(end-start)*i/steps;
        const x=Math.cos(a)*rx,y=Math.sin(a)*ry;i?c.lineTo(x,y):c.moveTo(x,y)}c.stroke();
    }
    // Residual droplets, only after the solid ribbon has begun to break up.
    if(q.residue>0){
      c.globalAlpha=q.residue*.8;c.fillStyle='#d7fffc';
      for(let i=0;i<12;i++){
        const a=i*Math.PI/6,near=Math.sin(a)>=0;if(near!==frontPass)continue;
        const spread=1+(t-.27)*.9,x=Math.cos(a)*rx*spread,y=Math.sin(a)*ry-h*(t-.27)*(.25+(i%3)*.12);
        c.beginPath();c.ellipse(x,y,h*.009,h*(.016+(i%3)*.007),-.2,0,Math.PI*2);c.fill();
      }
    }
    c.restore();
  }
...
function drawWaves(){for(const w of S.waves){
  ...
  if(w.k===1)continue; // Water is a body-centred sweep, not a detached projectile.
  if(w.k===2){
...
function bg(){
  ...
  if(image.complete&&image.naturalWidth>0){
    ...
    const scale=Math.max(W/image.naturalWidth,H*HORIZON/(image.naturalHeight*.18*1.28),H*(1-HORIZON)/(image.naturalHeight*.82*1.28));
    const width=image.naturalWidth*scale,height=image.naturalHeight*scale;
    // Distant scenery stays anchored; near ground moves by world distance.
    // No whole-scene zoom/reset: a ground point follows the combat projection.
    c.drawImage(image,W*.5-width*1.28*.5,H*HORIZON-height*1.28*.18,width*1.28,height*1.28);
    drawGround(image);
...
function draw(){
  ...
  if(posesReady())drawPoseHero(h);else if(rigReady())drawRig(h);else if(img.complete&&img.naturalWidth){const rect=crop(img),w=h*rect[2]/rect[3];c.drawImage(img,...rect,-w/2,-h,w,h)}c.restore();drawWaterEffect(true);drawEffects();c.restore();
...
function end(ok){
  ...
  if(window.GameBGM)window.GameBGM.pause();
  ...
  $('#previousHigh').textContent=previous.toLocaleString('ja-JP');
  $('#newRecord').textContent=ok&&points.total>previous?'NEW RECORD':'';
  const grade=ok?(S.t<=60?'SS':S.t<=90?'S':S.t<=120?'A':'B'):'—';
  const evaluation='<div class="group-evaluation"><span>まもののむれ討伐時の評価</span><b>'+grade+'</b><small>1/1</small></div>';
  $('#resultText').innerHTML=
    resultRow('クリアタイムスコア',ok?'今回のクリアタイム':'今回の経過時間',S.t.toFixed(2)+' 秒',points.time,evaluation)+
    resultRow('スキルフィニッシュスコア','今回のスキルフィニッシュ回数',S.fin,points.skill)+
    resultRow('総被ダメージスコア','今回の総被ダメージ',S.dmg.toLocaleString('ja-JP'),points.damage)+
    resultRow('与ダメージスコア','今回の与ダメージ',S.dealtDamage.toLocaleString('ja-JP'),points.dealt)+
    resultRow('オーバーキルボーナス','残りHPを超えたダメージ',S.overkill.toLocaleString('ja-JP'),points.overkill);
  $('#totalScore').textContent=points.total.toLocaleString('ja-JP');
  $('#result').classList.remove('hide');$('#result').scrollTop=0;
  if(window.bgm)bgm.pause();
}
function start(){if(window.GameSFX)window.GameSFX.start();reset();S.run=1;last=performance.now();$('#intro').classList.add('hide');if(window.bgm){bgm.currentTime=0;bgm.play().catch(()=>{})}}$('#start').onclick=start;$('#retry').onclick=()=>{if(window.GameSFX)window.GameSFX.start();reset();S.run=1;last=performance.now();$('#result').classList.add('hide');if(window.bgm)bgm.play().catch(()=>{})};// Own each gesture by pointerId; button fingers cannot finish a canvas swipe.
