...
(()=>{
  ...
  const ultimateAudio=new Audio('assets/ultimate-cutin-reference-tail-v2.mp3');ultimateAudio.preload='auto';let resumeUltimate=false;
  const buffers=new Map(),voices=new Set(),recent=new Map();
  const durations={ultimateCutin:3.584,waterStart:.18,waterSwing:.24,continentStart:.23,continentSwing:.34,hit:.12,down:.43,shot:.28,hurt:.20,battleStart:1.25,superdash:.48,guardRaise:.18,guardBlock:.24};
  ...
  function play(kind,type=''){
    ...
    try{
      if(kind==='ultimateCutin'){ultimateAudio.currentTime=Math.max(0,Math.min(3.584,Number(type)||0));ultimateAudio.play().catch(()=>{});return}
