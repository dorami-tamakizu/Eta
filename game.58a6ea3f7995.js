...
function sound(kind,type){if(window.GameSFX)window.GameSFX.play(kind,type)}function hurt(n){if(S.guard){S.guardImpact=.38;sound('guardBlock');return;}S.combo=0;S.shake=.18;burst(S.l,S.z+S.pd,'#ff7563',12);sound('hurt');S.hp-=n;S.dmg+=n;S.hit=.15;if(S.hp<=0)end(0)}function hit(e,n,sk,source=sk?'skill':'normal'){if(e.dead)return;const dealt=Math.min(Math.max(0,e.hp),Math.max(0,n));S.dealtDamage+=dealt;if(source==='normal'||source==='skill')S.ultimateCharge=C(S.ultimateCharge+dealt,0,ULTIMATE.capacity);S.overkill+=Math.max(0,n-Math.max(0,e.hp));sound('hit',e.t);e.hp-=n;e.fl=.12;S.combo++;S.comboTime=2;S.fx.push({kind:'number',l:e.l,z:e.z,n:n,t:0,life:.8,color:sk?'#fff079':'#ffffff'});burst(e.l,e.z,sk?'#b5f7ff':'#ffffff',9);S.shake=sk?.08:.035;if(e.tell>0&&!e.boss){e.tell=0;e.cd=R(1.8,3.8)}if(e.hp<=0){e.dead=1;e.fade=.4;burst(e.l,e.z,'#ffdfa0',18);sound('down',e.t);S.dashUntil=S.t+.5;S.dashReach=e.group!==undefined&&!S.en.some(o=>!o.dead&&o.group===e.group)?C(front()-S.z-S.pd-1.2,10,36):10;if(sk)S.fin++}}function hideNotice(){const n=$('#actionName');n.style.opacity='0';n.classList.remove('moveArrow');if(S)S.notice=null}
...
const ULTIMATE={capacity:90,damage:10,duration:1.87,cutin:.72,windup:.90,travel:.32};
...
function drawUltimate(){
  ...
  const charge=C((a.t-ULTIMATE.cutin)/(ULTIMATE.windup-ULTIMATE.cutin),0,1);
  const p=C((a.t-ULTIMATE.windup)/ULTIMATE.travel,0,1),fade=1-C((a.t-ULTIMATE.cutin-.60)/.55,0,1);
  const cx=W*.5,baseY=H*1.05,farY=H*(HORIZON+.008),tipY=baseY+(farY-baseY)*p;
  c.save();c.fillStyle='rgba(18,4,38,'+(.26*fade)+')';c.fillRect(0,0,W,H);c.globalCompositeOperation='lighter';
  if(p<=0){
    c.globalAlpha=charge;c.strokeStyle='#d691ff';c.lineWidth=4;c.beginPath();c.ellipse(cx,H*.87,W*(.06+.13*charge),H*.025,0,0,Math.PI*2);c.stroke();
  }else{
    const pulse=1+.025*Math.sin(a.t*85);
    // One broad, centered perspective beam; its outer envelope covers all three lanes.
    for(const [half,color,alpha] of [[.53,'#7130ff',.25],[.44,'#8f32ff',.48],[.34,'#bd65ff',.8],[.25,'#dfadff',.94],[.17,'#fff2ff',1]]){
      const w=W*half*pulse,tipW=w*(1-.90*p);
      c.globalAlpha=fade*alpha;c.fillStyle=color;c.beginPath();c.moveTo(cx-w,baseY);c.lineTo(cx+w,baseY);c.lineTo(cx+tipW,tipY);c.quadraticCurveTo(cx,tipY-H*.025,cx-tipW,tipY);c.closePath();c.fill();
    }
    // Gold lightning arcs surround the single violet/white beam, matching its icon.
    c.strokeStyle='#ffe381';c.lineWidth=2;c.globalAlpha=fade*.92;
    for(let j=0;j<7;j++){
      const u=((a.t*1.7+j/7)%1)*p,y=baseY+(farY-baseY)*u,rx=W*.43*(1-.90*u);
      c.beginPath();c.ellipse(cx,y,rx,rx*.22,-.15,Math.PI*(.15+j%2),Math.PI*(.92+j%2));c.stroke();
    }
    const burst=W*.055*(1+.15*Math.sin(a.t*90));c.globalAlpha=fade;c.fillStyle='#fff5df';c.beginPath();c.ellipse(cx,tipY,burst,burst*.6,0,0,Math.PI*2);c.fill();
  }
  c.restore();
}
function fire(k){
...
$('#resume').onclick=()=>{if(window.GameSFX)window.GameSFX.start();S.pause=0;if(ultimateCutinActive())sound('ultimateCutin',S.ultimate.t);last=performance.now();$('#pause').classList.add('hide');if(window.GameBGM)window.GameBGM.resume()};
cv.onpointerdown=e=>{
