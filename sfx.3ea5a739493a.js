// Local trimmed reference start cue and procedural gameplay effects.
(()=>{
  let context=null,bus=null,enabled=false,epoch=0;
  const battleAudio=new Audio('assets/battle-start-tail.mp3');battleAudio.preload='auto';
  const ultimateAudio=new Audio('assets/ultimate-cutin-reference-v1.mp3');ultimateAudio.preload='auto';
  const buffers=new Map(),voices=new Set(),recent=new Map();
  const durations={ultimateCutin:.72,waterStart:.18,waterSwing:.24,continentStart:.23,continentSwing:.34,hit:.12,down:.43,shot:.28,hurt:.20,battleStart:1.25,superdash:.48,guardRaise:.18,guardBlock:.24};
  ...
  function ensure(){
    ...
    context.createMediaElementSource(battleAudio).connect(bus);
    context.createMediaElementSource(ultimateAudio).connect(bus);
    bus.connect(limiter);limiter.connect(context.destination);return true;
  }
  function clear(){
    battleAudio.pause();battleAudio.currentTime=0;
    ultimateAudio.pause();ultimateAudio.currentTime=0;
  ...
  function play(kind,type=''){
    ...
    try{
      if(kind==='ultimateCutin'){ultimateAudio.currentTime=Math.max(0,Math.min(.72,Number(type)||0));ultimateAudio.play().catch(()=>{});return}
