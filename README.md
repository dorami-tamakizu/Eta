...
タイトル画面のメニューは「ゲーム説明」「ランキング」の2項目です。ランキングはSupabaseの全プレイヤー共通の上位100件です。クリア結果の「名前を入力してランキング登録」から任意の名前で投稿し、各記録の詳細で4項目の数値・得点とオーバーキルを表示します。ログインやプレイヤーIDはありません。同名の投稿も別記録として扱います。
