...
code=code.replace('reset();function loop(t)','globalThis.test={reset,step,hit,accelerate,fire,score,drawPoseHero,get state(){return S}};reset();function loop(t)');
...
let s=setup();tick(50);assert(Math.abs(s.z-5/3)<1e-8);const base=s.z;
s=setup();g.accelerate();tick(50);assert(Math.abs(s.z-2*base)<1e-8);tick(60);assert.equal(s.boost,0);
...
s=setup();s.en[0].z=2;s.dashUntil=.5;g.accelerate();tick(10);assert(s.z<=s.en[0].z-.7+1e-8);assert.equal(s.dash,null);
s=setup();s.en[0].z=100;s.dashUntil=.5;g.accelerate();tick(24);assert(s.z<=10+1e-8);assert.equal(s.dash,null);
