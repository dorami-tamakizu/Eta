...
const packInterval=groupStart(1)-groupStart(0),dashDistance=20;assert(dashDistance<packInterval-3,'dash ends before crossing the empty gap between packs');
...
g.reset();s=g.state;s.run=1;s.z=20;s.pd=0;s.pt=0;for(const e of s.en)if(e.group===0)e.dead=1;s.dashUntil=.5;swipe();tick(dashDistance/42);assert.equal(s.dash,null);const nextPack=s.en.find(e=>e.group===1);assert(nextPack.z-position()>8,'dash expires before reaching the next live pack');const beforeStride=position();swipe();tick(.2);assert(position()-beforeStride>9*.2,'manual upward swipe speeds up the remaining approach');
console.log('PASS: partial-pack-interval dash cap, frame-rate independence, normal-speed fallback, early contact, pause, single reward, and boss distance cap');
