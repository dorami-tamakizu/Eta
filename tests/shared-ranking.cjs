...
const record={id:'record-one',name:'<img src=x onerror=alert(1)>',clear_time:50,skill_finishes:60,damage_taken:12,damage_dealt:308,overkill:94,time_score:3200000,skill_score:393443,damage_taken_score:100000,damage_dealt_score:143925,total_score:3837368};
...
(async()=>{
 ...
 $('#playerName').value=' A ';const pending=$('#scoreEntryForm').onsubmit({preventDefault(){}});await $('#scoreEntryForm').onsubmit({preventDefault(){}});assert.equal(calls.length,1);assert.equal(JSON.parse(calls[0].opts.body).name,'A');assert.equal(JSON.parse(calls[0].opts.body).overkill,94);assert.equal(JSON.parse(calls[0].opts.body).total_score,undefined);
 ...
 g.setResult(true,data);$('#playerName').value='B';const failure=$('#scoreEntryForm').onsubmit({preventDefault(){}});resolvePost({ok:false,status:400});await failure;assert.equal($('#submitScore').disabled,false);
