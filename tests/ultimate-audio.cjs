...
s.start();s.play('ultimateCutin',.35);assert.equal(audio.currentTime,.35);assert.equal(audio.plays,2);
s.pause();s.start();s.play('ultimateCutin',0);assert.equal(audio.currentTime,0);
