...
const gains=[];const node=()=>({connect(){},disconnect(){},gain:{value:1}});
class Context{constructor(){this.state='running';this.destination={}}createGain(){const g=node();gains.push(g);return g}createDynamicsCompressor(){return {...node(),threshold:{},knee:{},ratio:{},attack:{},release:{}}}createMediaElementSource(){return node()}resume(){this.state='running';return Promise.resolve()}suspend(){this.state='suspended';return Promise.resolve()}}
...
assert.equal(gains[1].gain.value,.06,'battle cue volume controlled through Web Audio');
