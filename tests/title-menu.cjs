...
g.reset();g.openTitlePanel('help');assert.equal(g.state.run,0);assert.equal(nodes.get('#titlePanelHeading').textContent,'ゲーム説明');assert(nodes.get('#titlePanelBody').innerHTML.includes('1秒に1回'));g.closeTitlePanel();g.openTitlePanel('ranking');assert(nodes.get('#titlePanelBody').innerHTML.includes('まだクリア記録'));
