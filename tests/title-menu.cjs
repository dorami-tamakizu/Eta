...
g.reset();g.openTitlePanel('help');assert.equal(g.state.run,0);assert(nodes.get('#guideImage').alt.includes('ゲーム概要'));assert.equal(nodes.get('#guidePrev').hidden,true);nodes.get('#guideNext').onclick();assert(nodes.get('#guideImage').alt.includes('スーパーダッシュ'));nodes.get('#guideNext').onclick();assert.equal(nodes.get('#guideExit').textContent,'閉じる');assert.equal(nodes.get('#guideNext').hidden,false);nodes.get('#guideNext').onclick();assert(nodes.get('#guideImage').alt.includes('ランキングに登録'));assert.equal(nodes.get('#guideNext').hidden,true);assert.equal(nodes.get('#guideCount').textContent,'4 / 4');assert.equal(nodes.get('#guideExit').textContent,'閉じる');nodes.get('#guidePrev').onclick();assert(nodes.get('#guideImage').alt.includes('ハイスコア'));assert.equal(nodes.get('#guideCount').textContent,'3 / 4');g.closeGuide();g.openTitlePanel('help');assert(nodes.get('#guideImage').alt.includes('ゲーム概要'));g.closeGuide();g.openTitlePanel('ranking');assert(nodes.get('#titlePanelBody').innerHTML.includes('shared ranking'));
