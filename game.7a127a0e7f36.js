...
// End before the next pack so the remaining approach rewards manual swipe strides.
const SUPERDASH_DISTANCE=ENEMY_GROUP_GAP*.625,SUPERDASH_SPEED=42;
...
function adv(n){
  if(S.phase==='boss'){
    // Only the player moves in the fixed boss arena; never move the camera or boss.
...
function updateBoss(e,dt){
  ...
  // Fixed position and stance: player retreat/advance must not move the enemy.
  e.displayLane=e.l;e.stepClock=0;e.stepFrame=0;
  e.moveActive=false;e.backstepActive=false;e.backstepPhase=0;
