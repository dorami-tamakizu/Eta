...
function notice(text,seconds,arrow=false){
  ...
  if(ultimatePlate){n.innerHTML='<span class="ultimate-kicker">奥 義</span><span class="ultimate-prefix">渾身の</span><span class="ultimate-title">ストラッシュ</span>';}else if(plate){
...
function placeNotice(){
  ...
  if(state.text==='渾身のストラッシュ'){n.style.left=W/2+'px';n.style.top=H*.43+'px';n.style.transform='translate(-50%,-50%)';n.style.opacity=String(Math.min(1,age/.05,state.left/.18));return;}
...
function updateBoss(e,dt){
  ...
  if(e.action){
    const a=e.action;if(a.kind==='wide'&&a.targetZ===undefined)a.targetZ=S.z+S.pd;a.age+=dt;e.tell=a.kind==='normal'?0:Math.max(0,a.impact-a.age);
  ...
  if(e.wideCd<=0){
    e.wideCd=R(2.6,4.4);e.action={kind:'wide',l:1,targetZ:S.z+S.pd,age:0,impact:.5,duration:1.0,done:false};e.tell=.5;e.attackCount++;return;
  }
  if(e.dragonCd<=0){
