...
function updateBoss(e,dt){
  ...
  const p=e.patrol,target=route[p.leg],duration=Math.max(3,Math.abs(target[0]-p.fromLane)/.7,Math.abs(target[1]-p.fromDepth)/1.2);
  p.t=Math.min(duration,p.t+dt);const q=E(p.t/duration),oldLane=e.displayLane??p.fromLane;
  ...
  if(e.moveActive)e.backstepPhase=((e.backstepPhase||0)+(Math.abs(movement)+Math.abs(lateral)*2)/1.4*(movement<0?-1:1)+1)%1;
...
function drawBossSprite(e,h){
  ...
  drawBossAura(e,h);
  const walk=im.bossOverhead;
  const swinging=a&&a.age>=a.impact&&a.age<a.impact+.32;
  if(swinging&&atlas.complete&&atlas.naturalWidth){
    const frame=a.kind==='dragon'?6:Math.min(6,3+Math.floor((a.age-a.impact)/.08));
  ...
  }else if(walk.complete&&walk.naturalWidth){
    const frame=e.moveActive?Math.floor((e.backstepPhase||0)*8)%8:0,cw=walk.naturalWidth/4,ch=walk.naturalHeight/2,w=h*cw/ch;
    c.drawImage(walk,(frame%4)*cw,Math.floor(frame/4)*ch,cw,ch,-w/2,-h,w,h);
  }else if(atlas.complete&&atlas.naturalWidth){
    const cw=atlas.naturalWidth/4,ch=atlas.naturalHeight/2,w=h*cw/ch;c.drawImage(atlas,2*cw,0,cw,ch,-w/2,-h,w,h);
  }
