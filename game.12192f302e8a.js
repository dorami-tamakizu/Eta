...
function fire(k){
  if(!S.run||S.pause||S.cinematic||S.skill||S.ch[k-1]<1)return;
...
function lockCinematic(kind){
  ...
  S.skill=null;S.normalAttack=null;S.norm=0;S.dash=null;S.dashUntil=-1;S.boost=0;
...
function step(dt){
  ...
  if(!S.introRun&&!S.en.some(e=>!e.dead)){if(S.phase==='road')startBoss();else startVictory();}
  ui();
}
...
// Both legs use identical geometry and gait, offset by exactly half a cycle.
function runLegPose(phase){
  const swing=Math.sin(phase),lift=(1+swing)/2;
  return{thigh:102-22*lift,shin:164-94*lift,angle:.025+.055*Math.cos(phase),lift};
}
function drawBalancedRun(h){
  const img=im.heroRun,phase=S.runPhase,scale=h/480;
  const bob=3*Math.cos(phase*2);
  c.save();c.scale(scale,scale);c.translate(-187,-478+bob);
  // Reuse one clean armored leg for both sides, keeping sword/shield handedness.
  const legs=[{x:155,side:1,p:phase},{x:217,side:-1,p:phase+Math.PI}];
  legs.sort((a,b)=>runLegPose(b.p).lift-runLegPose(a.p).lift);
  for(const leg of legs){
    const q=runLegPose(leg.p);c.save();c.translate(leg.x,207);c.scale(leg.side,1);c.rotate(q.angle);
    c.save();c.scale(1,q.thigh/91);segment(img,[120,195,80,109],[159,207],[176,296],91);c.restore();
    c.translate(0,q.thigh-5);c.rotate(-q.angle*.4);
    c.save();c.scale(1,q.shin/165);segment(img,[142,288,66,190],[176,296],[187,461],165);c.restore();
    c.restore();
  }
  // Upper body, cape, shield and sword retain their original orientation.
  c.save();c.beginPath();c.moveTo(0,0);c.lineTo(384,0);c.lineTo(384,512);c.lineTo(273,512);c.lineTo(273,220);c.lineTo(248,207);c.lineTo(121,207);c.lineTo(113,250);c.lineTo(0,250);c.closePath();c.clip();
  c.drawImage(img,0,0,384,512,0,0,384,512);c.restore();c.restore();
}
...
for(const id of ['#guard','#s1','#s2']){
  ...
  button.onpointerdown=e=>{
    ...
    if(id==='#guard'){
      if(S.run&&!S.pause&&!S.cinematic){S.guard=1;sound('guardRaise');button.setPointerCapture(e.pointerId);notice('防御',.5)}
    }else fire(id==='#s1'?1:2);
  };
}
...
addEventListener('keydown',e=>{if(!S.run||S.pause||S.cinematic)return;if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','1','2',' '].includes(e.key))e.preventDefault();if(e.repeat)return;if(e.key==='ArrowLeft')lane(-1);if(e.key==='ArrowRight')lane(1);if(e.key==='ArrowUp')accelerate();if(e.key==='ArrowDown')depth(-MAX_DEPTH/4);if(e.key==='1')fire(1);if(e.key==='2')fire(2);if(e.key===' '){S.guard=1;sound('guardRaise')}});addEventListener('keyup',e=>{if(e.key===' ')S.guard=0});
