...
function notice(text,seconds,arrow=false){
  ...
  if(ultimatePlate){n.innerHTML='<span class="ultimate-title">'+text+'</span>';}else if(plate){
...
function updateBoss(e,dt){
  ...
  if(e.action){
    ...
    if(!a.done&&a.age>=a.impact){
      ...
      if(a.kind==='dragon'){
        ...
        if(a.kind==='wide'){for(let lane=0;lane<3;lane++)burst(lane,S.z+S.pd+1,'#ffae43',8);if(distance(e)>=0&&distance(e)<=25)hurt(3);}
        else if(S.l===a.l&&distance(e)>=0&&distance(e)<=3)hurt(3);
      }
    }
    ...
    e.cd-=dt;
    if(e.cd<=0){
      const kind=random()<.30?'wide':(distance(e)>4||e.attackCount%3===2?'dragon':'sword');
      e.attackCount++;e.l=S.l;
      e.action={kind,l:S.l,age:0,impact:kind==='wide'?.5:kind==='dragon'?1.5:1.1,duration:kind==='wide'?1.25:kind==='dragon'?2.1:2.0,done:false};
  ...
  if(e.backstepActive)e.backstepPhase=((e.backstepPhase||0)+backward/5)%1;
}
function drawBossSprite(e,h){
  ...
  if(e.backstepActive&&walk.complete&&walk.naturalWidth){
  ...
  }else if(a&&atlas.complete&&atlas.naturalWidth){
    let frame;
...
function bossAuraProgress(e){
  const a=e.action;if(!a||a.done||e.dead)return -1;
...
function drawBossTelegraph(){
  ...
  const near=S.pd+.1,far=e.z-S.z,half=Math.min(W,520)*.115,lanes=a.kind==='wide'?[0,1,2]:[a.l];
  c.save();c.fillStyle='rgba(255,65,18,'+(.10+.10*(a.age/a.impact))+')';c.strokeStyle='#ffa244';c.lineWidth=1.5;
  for(const lane of lanes){c.beginPath();c.moveTo(lx(lane,near)-half*perspective(near),yy(near));c.lineTo(lx(lane,near)+half*perspective(near),yy(near));c.lineTo(lx(lane,far)+half*perspective(far),yy(far));c.lineTo(lx(lane,far)-half*perspective(far),yy(far));c.closePath();c.fill();c.stroke();}
  c.font='bold 14px sans-serif';c.textAlign='center';c.fillStyle='#ffe2a4';c.fillText(a.kind==='wide'?'全列斬撃 — 防御':a.kind==='dragon'?'炎竜':'剣撃',lx(a.kind==='wide'?1:a.l,near),yy(near)+20);c.restore();
}
function drawBossWideSlash(){
  const e=S.en.find(e=>e.boss&&!e.dead),a=e&&e.action;if(!a||a.kind!=='wide'||!a.done||S.cinematic)return;
  const q=(a.age-a.impact)/.25;if(q<0||q>1)return;
  c.save();c.globalAlpha=1-q;c.strokeStyle='#fff5c7';c.shadowColor='#ff620c';c.shadowBlur=20;c.lineWidth=12*(1-q)+2;
  c.beginPath();c.moveTo(-W*.08,H*.60);c.quadraticCurveTo(W*.50,H*(.40+.10*q),W*1.08,H*.56);c.stroke();
  c.strokeStyle='#ff8730';c.lineWidth=4;c.beginPath();c.moveTo(-W*.08,H*.64);c.quadraticCurveTo(W*.5,H*(.45+.10*q),W*1.08,H*.60);c.stroke();c.restore();
}
...
function draw(){
  ...
  if(armoredReady())drawArmoredHero(h);else if(img.complete&&img.naturalWidth){const rect=crop(img),w=h*rect[2]/rect[3];c.drawImage(img,...rect,-w/2,-h,w,h)}c.restore();drawWaterEffect(true);drawEffects();c.restore();drawUltimate();drawBossWideSlash();drawBattleBanner();
