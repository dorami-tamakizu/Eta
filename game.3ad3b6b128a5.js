...
function drawWaterEffect(frontPass){
  const sheet=im.waterSheet;
  if(!sheet.complete||!sheet.naturalWidth)return;
  const cw=sheet.naturalWidth/4,ch=sheet.naturalHeight/2;
  // The rear water wall goes behind the hero; the near ribbon crosses in front.
  const split=ch*.62;
  for(const f of S.waterFx){
    const t=f.age,frame=Math.min(7,Math.floor(t/.075));
    const h=playerHeight(),w=h*1.72,scale=w/cw;
    const x=lx(f.l,S.pd)-w/2,y=playerY()-h*.48-split*scale;
    const sy=frontPass?split:0,sh=frontPass?ch-split:split;
    const alpha=Math.min(1,t/.018)*C((.60-t)/.075,0,1);
    c.save();c.globalAlpha=alpha;
    c.drawImage(sheet,(frame%4)*cw,Math.floor(frame/4)*ch+sy,cw,sh,
      x,y+sy*scale,w,sh*scale);
    c.restore();
  }
}
