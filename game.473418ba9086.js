...
function adv(n){
  if(S.phase==='boss'){
    ...
    const target=Math.max(S.pd,Math.min(depthLimit(),S.dash||S.boost>0?DASH_DEPTH:RUN_DEPTH));
    const move=Math.min(target-S.pd,n*.65);S.pd+=move;S.pt=S.pd;return move;
  }
