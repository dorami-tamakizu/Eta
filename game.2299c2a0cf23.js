...
const HORIZON=.26,GROUND_SPAN=.49;
const MAX_DEPTH=(GROUND_SPAN/(.46-HORIZON)-1)/.115, FRONT_GAP=.7;
// Reference framing: horizon 26%, running feet 52%, body above center.
const RUN_DEPTH=(GROUND_SPAN/(.52-HORIZON)-1)/.115,DASH_DEPTH=(GROUND_SPAN/(.48-HORIZON)-1)/.115;
...
function step(dt){
  ...
  if(!S.guard){
    ...
    else adv((dt+Math.min(dt,S.boost))*(1/.3));
  }
  S.boost=Math.max(0,S.boost-dt);
  const travelled=S.z+S.pd-previousZ;S.speed=dt>0?travelled/dt:0;S.runPhase+=Math.max(0,travelled)*2.8;
...
function bg(){
  ...
  if(image.complete&&image.naturalWidth>0){
    ...
    const paint=z=>c.drawImage(image,W*.5-width*z*.5,H*HORIZON-height*z*.18,width*z,height*z);
    paint(zoom);
