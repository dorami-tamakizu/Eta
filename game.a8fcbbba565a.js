(()=>{const $=s=>document.querySelector(s),cv=$('#game'),c=cv.getContext('2d');let W,H,last=0,S,combatSeed=17319;function random(){combatSeed=(Math.imul(combatSeed,1664525)+1013904223)>>>0;return combatSeed/4294967296}const C=(v,a,b)=>Math.max(a,Math.min(b,v)),R=(a,b)=>a+random()*(b-a),RI=(a,b)=>Math.floor(R(a,b+1)),E=t=>t<.5?2*t*t:1-Math.pow(-2*t+2,2)/2;const D={slime:{hp:2,d:1,r:0,h:205},skeleton:{hp:3,d:2,r:0,h:220},mage:{hp:3,d:2,r:1,h:228},bonobo:{hp:4,d:4,r:0,h:245},dragon:{hp:4,d:3,r:1,h:215},boss:{hp:40,d:3,r:0,h:300}},im={};const src={boss:'assets/boss-warlord.png',bossSword:'assets/boss-sword-atlas.png',fireDragon:'assets/boss-fire-dragon.png',hero:'assets/hero.webp',slime:'assets/suraimu.png',skeleton:'assets/がいこつ.png',mage:'assets/mahoutukai.png',bonobo:'assets/bonobo.png',dragon:'assets/minidoragonn.png',body:'hero_body.png.png',upper:'hero_arm1.png',fore:'hero_arm2.png',hand:'hero_hand.png',shoulder:'hero_shoulder.png',sword:'assets/hero_sword_motion.png',forest:'assets/forest-background.webp',forestTrees:'assets/forest-trees-gpt-v1.webp',forestGround:'assets/forest-ground-gpt-v1.webp',poses:'assets/hero-motion-sheet.png',waterSheet:'assets/water-slash-gpt-v3.webp',earthSheet:'assets/earth-slash-gpt-v1.webp',heroSkills:'assets/hero-skills-gpt-v1.webp',heroRun:'assets/hero-run-female-v1.png',heroGuard:'assets/hero-guard-gpt-v1.webp',guardImpact:'assets/guard-impact-gpt-v1.webp',dashTrail:'assets/superdash-gpt-v1.webp'};for(const k in src){im[k]=new Image;im[k].src=src[k]}
...
const ULTIMATE={capacity:30,damage:10,duration:1.15,windup:.18,travel:.32};
function updateUltimateUI(){
  const value=C(S.ultimateCharge,0,30),button=$('#ultimate');
  $('#ultimateFill').style.width=(value/30*100)+'%';
  $('#ultimateValue').textContent=value>=30?'MAX':Math.floor(value)+'/30';
...
function drawUltimate(){
  ...
  const launch=C(a.t/ULTIMATE.windup,0,1),p=C((a.t-ULTIMATE.windup)/ULTIMATE.travel,0,1);
  const fade=1-C((a.t-.60)/.55,0,1);
...
function accelerate(){
  if(!S.run||S.pause||S.cinematic)return;
  if(S.dashUntil>=0&&S.t<=S.dashUntil+1e-9){
    S.dash={n:S.dashReach||10};sound('superdash');S.dashUntil=-1;S.boost=0;
  }else S.boost=1;
}
function depth(d){if(S.cinematic)return;S.retreat=1;S.pt=C(S.pd+d,0,depthLimit())}
...
function startVictory(){
  if(S.cinematic)return;S.phase='victory';lockCinematic('victory');
}
...
function drawArmoredHero(h){
  const action=S.ultimate?{k:1,t:S.ultimate.t,d:ULTIMATE.duration}:S.skill||S.normalAttack;
...
cv.onpointerdown=e=>{
  if(!S.run||S.pause||S.cinematic||S.ptr)return;
