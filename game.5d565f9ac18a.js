...
function fire(k){
  ...
  if(S.skill&&S.skill.waterVisual)S.waterFx=S.waterFx.filter(f=>f!==S.skill.waterVisual);
...
function step(dt){
  ...
  if(S.skill){
    ...
    if(a.k===1){
      ...
      if(a.waterVisual)a.waterVisual.sweep=C((limit-.40)/.32,0,1);
...
function drawWaterEffect(frontPass){
  ...
  for(const f of S.waterFx){
    ...
    const sweep=f.sweep??1;c.beginPath();c.rect(lx(f.l,S.pd)+w/2-w*sweep,0,w*sweep,H);c.clip();
