...
(()=>{
  ...
  const battleAudio=new Audio('assets/battle-start-syakiin-v8.mp3');battleAudio.preload='auto';
  const ultimateAudio=new Audio('assets/ultimate-syakiin-heavy-v9.mp3');ultimateAudio.preload='auto';let resumeUltimate=false;
  const flightAudio=new Audio('assets/ultimate-sword-flight-v10.mp3');flightAudio.preload='auto';let resumeFlight=false;
  const fallAudio=new Audio('assets/hero-fall-v11.mp3');fallAudio.preload='auto';let resumeFall=false;
  const buffers=new Map(),voices=new Set(),recent=new Map();
  const durations={heroFall:1.803,ultimateFlight:3.109,ultimateCutin:3.214,waterStart:.18,waterSwing:.24,continentStart:.23,continentSwing:.34,hit:.12,down:.43,shot:.28,hurt:.20,battleStart:5.329,superdash:.48,guardRaise:.18,guardBlock:.24};
  ...
  function ensure(){
    ...
    const battleGain=context.createGain();battleGain.gain.value=.06;
    context.createMediaElementSource(battleAudio).connect(battleGain);battleGain.connect(context.destination);
    context.createMediaElementSource(ultimateAudio).connect(context.destination);
    context.createMediaElementSource(flightAudio).connect(context.destination);
    context.createMediaElementSource(fallAudio).connect(context.destination);
  ...
  function start(){
    ...
    try{
      ...
      if(resumeFlight){resumeFlight=false;flightAudio.play().catch(()=>{});}
      if(resumeFall){resumeFall=false;fallAudio.play().catch(()=>{});}
  ...
  function pause(){
    ...
    resumeFlight=resumeFlight||(!flightAudio.paused&&!flightAudio.ended);flightAudio.pause();
    resumeFall=resumeFall||(!fallAudio.paused&&!fallAudio.ended);fallAudio.pause();
  ...
  function play(kind,type=''){
    ...
    try{
      ...
      if(kind==='heroFall'){fallAudio.currentTime=0;fallAudio.play().catch(()=>{});return}
      if(kind==='ultimateFlight'){flightAudio.currentTime=0;flightAudio.play().catch(()=>{});return}
