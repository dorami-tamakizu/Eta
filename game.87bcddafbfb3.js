...
function drawUltimate(){
  ...
  const p=C((a.t-ULTIMATE.windup)/ULTIMATE.travel,0,1),fade=1-C((a.t-ULTIMATE.cutin-.40)/(.55*2/3),0,1);
...
const SCORE_KEY='eta.score.v4.skill-time';
function score(ok){
  // 100,000 theoretical maximum: speed 80%, safety 15%, finishes 3%, progress 2%.
  const time=ok?Math.round(80000*Math.exp(-Math.max(0,S.t)/240)):0;
  const damage=ok?Math.round(15000*Math.pow(C(1-S.dmg/20,0,1),2)):0;
  const skill=Math.round(3000*C(S.fin/61,0,1));
  const dealt=Math.round(2000*C(S.dealtDamage/214,0,1)),overkill=0;
